$(() => {
  ...
  // let diary_no = queryString.split("=")[1]; //=로 split으로 나눈 1번 인덱스
    ...
    $imageObj.hide();
      ...
      $(jsonarr).each((i, element) => {
        ...
        $copyObj
        .find("span.clientId")
        .html(jsonarr[i].client.clientId);
        $copyObj.find("span.clientId").hide();

        $copyObj
        .find("span.commentNo")
        .html(jsonarr[i].commentNo);
    ...
    if(jsonObj.t.loginedId == null || jsonObj.t.loginedId==""){ //로그인 안된경우 -
      $likeImgObj.hide();
    }else{
      $likeImgObj.show();
      if(!jsonObj.t.likeFlag){ //로그인 되고 좋아요 누르지 않은 경우
        $likeImgObj.attr("src", "../images/unlike.png"); 
      }else{                   //로그인 되고 좋아요 누른 경우
  ...
  $("div.comments>div.comment>button.insert").click((e) => {
    let commentContent = $(e.target).siblings('textarea[name=commentContent]').val()
    ...
    $("div.comments").on(
      "click", 
      "div.comment_view>button.update",
      (e) => {
        if($(e.target).siblings('input.commentContent').attr("readonly") !== undefined ){
          $(e.target).siblings('input.commentContent').attr({readonly : false, onfocus : false});
          $(e.currentTarget).html("수정완료");
          alert("댓글을 수정하세요");
        }
        else{
          $(e.target).siblings('input.commentContent').attr({readonly : "readonly", onfocus : "this.blur()"});
      ...
      $.ajax({
        url: "http://localhost:9997/back/diary/" + diaryNo + "/comment",
        method: "put",
        headers: {
          "content-Type": "application/json"
        },
    ...
    $("div.comments").on(
      "click", 
      "div.comment_view>button.delete",
      (e) => {
        let commentNo = $(e.target).siblings('span.commentNo').html();
        console.log("댓글번호:" + commentNo);
    ...
    // let diaryNo = 1;
    let jsondata = {"diaryNo":diaryNo, "commentNo":commentNo, "client":{"clientId":clientId}}
      alert("삭제되었습니다.");
    ...
    //--좋아요--
    $("div.like_view>span.like>img").on("click", function(e) {
      ...
      $.ajax({
        url: "http://localhost:9997/back/like/" + likeStatus,
        method: "post",
        contentType: "application/json",
        data: data,
        success: (jsonObj) => {
