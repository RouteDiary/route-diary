...
public class DiaryController {
  ...
  public ResultBean<?> modifyDiary(@RequestPart List<MultipartFile> imageFiles,
      @RequestPart Diary diary) throws DiaryException {
    ResultBean<?> resultBean = new ResultBean<>();
    try {
      diaryService.modifyDiary(diary, imageFiles);
      resultBean.setStatus(1);
      resultBean.setMessage("다이어리 수정에 성공하였습니다.");
    } catch (Exception e) {
      resultBean.setStatus(0);
      resultBean.setMessage("다이어리 수정에 실패하였습니다. \n" + e.getMessage());
    }
    return resultBean;
  }
  ...
  @GetMapping(value = "diary/mylist/{pageNo}")
  public ResultBean<PageBean<Diary>> showMyDiaryBoard(@PathVariable int pageNo, HttpSession session)
      throws DiaryException {
    ResultBean<PageBean<Diary>> resultBean = new ResultBean<PageBean<Diary>>();
    try {
      String loginedId = (String) session.getAttribute("loginInfo");
      loginedId = "koreaman@gmail.com";
      PageBean<Diary> pageBean = diaryService.showMyDiaryBoard(pageNo, loginedId);
      resultBean.setStatus(1);
  ...
  @GetMapping("diary/list/{order}/{pageNo}")
  public ResultBean<PageBean<Diary>> showDiaryBoard(@PathVariable int pageNo,
      @PathVariable int order, @RequestBody List<String> hashtags) {
    ResultBean<PageBean<Diary>> resultBean = new ResultBean<PageBean<Diary>>();
    try {

      PageBean<Diary> pageBean = diaryService.showDiaryBoard(order, pageNo, hashtags);
      resultBean.setStatus(1);
      resultBean.setT(pageBean);
  ...
  public ResponseEntity<?> writeComment(@PathVariable int diaryNo,
      // @RequestBody Comment comment,
      String commentContent, HttpSession session) throws DiaryException {
    // 유효성 검사
    ...
    try {
      ...
      e.printStackTrace();
      return new ResponseEntity<>(e.getMessage(), HttpStatus.INTERNAL_SERVER_ERROR); // 500에러
    }
  }
  ...
  public ResponseEntity<Object> modifyComment(@PathVariable int diaryNo,
    ...
    try {
      if (loginedId.equals(comment.getClient().getClientId())) {
        ...
        return new ResponseEntity<>("내 댓글이 아닌데..", HttpStatus.BAD_REQUEST); // 400에러
      }
  ...
  public ResponseEntity<?> removeComment(@PathVariable int diaryNo, @RequestBody Comment comment,
    ...
    try {
      if (loginedId.equals(comment.getClient().getClientId())) {
        ...
        return new ResponseEntity<>("삭제 실패ㅠㅠ", HttpStatus.BAD_REQUEST);
      }
      return new ResponseEntity<>(HttpStatus.OK);
    } catch (RemoveException e) {
      e.printStackTrace();
